...
class MARGOTModel(BaseEstimator):
    ...
    def fit(self, X: np.array =None, y: np.array =None, debug : bool = False):
        ...
        if self.max_depth == 3:
            f = quicksum(w[j, t] * w[j, t] for t in T_b for j in range(len(X[0]))) + self.alpha_0*quicksum(e[i, 0] for i in range(len(X)))+self.alpha_1*quicksum(e[i, t] for i in range(len(X)) for t in [1, 2, 5, 8, 9, 12])
...
if __name__ == '__main__':
    ...
    for seed in [0]:
        ...
        margot_gaps.append(best_mio.model.MIPGap)
        margot_runtimes.append(best_mio.model.Runtime)
        margot_n_weights.append(best_mio.mean_n_weights)
        ...
        margot_trains.append(train_acc)
        margot_tests.append(test_acc)
    ...
    print("MARGOT Train: ", np.mean(margot_trains), " +- ", np.std(margot_trains))
    print("MARGOT Test: ", np.mean(margot_tests), " +- ", np.std(margot_tests))
    print("MARGOT gaps: ", np.mean(margot_gaps), " +- ", np.std(margot_gaps))
    print("MARGOT times: ", np.mean(margot_runtimes), " +- ", np.std(margot_runtimes))
    print("MARGOT mean number of weights per branch node: ", np.mean(margot_n_weights), " +- ", np.std(margot_n_weights))
    ...
    result_line.append(str(round(np.mean(margot_trains), 2)) + " $\pm$ "+str(round(np.std(margot_trains), 2)))
    result_line.append(str(round(np.mean(margot_tests), 2)) + " $\pm$ "+str(round(np.std(margot_tests), 2)))
